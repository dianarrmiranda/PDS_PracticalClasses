...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		if (args.length == 0) new JGalo(new JGaloMethods());
		else if (args.length == 1) {
			if (args[0].charAt(0) == 'X') {
				new JGalo(new JGaloMethods('X'));
			} else if (args[0].charAt(0) == 'O') {
				new JGalo(new JGaloMethods('O'));
			} else {
				System.err.println("Invalid player name! (Start with either X or O)");
			}
		} else System.err.println("Invalid number of arguments!");
	}
} 
